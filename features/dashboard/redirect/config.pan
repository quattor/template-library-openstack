...
# This template CANNOT be used at the same time as features/dashboard/wsgi/config as 
# it manages the same file
unique template features/dashboard/redirect/config;
...
'contents/server_name' = OS_HORIZON_NEW_HOST;
'contents/server_aliases' = if ( OS_HORIZON_NEW_HOST != FULL_HOSTNAME) {
    list(FULL_HOSTNAME);
} else {
    null;
};
...
'config' = file_contents('features/dashboard/redirect/horizon.tt');
'perms' = '0644';
